...
//        $this->layout && $this->app->view->engine()->layout($this->layout);
    ...
    protected function buildTableParames($excludeFields = [])
    {
        ...
        // 判断是否关联查询
        $tableName = MyToolkit::humpToLine(lcfirst($this->model->getName()));
    ...
    public function selectList()
    {
        ...
        if ($whereStr = input('selectWhere')) {
            $whereArr = json_decode($whereStr, true);
            foreach ($whereArr as $item) {
                $this->selectWhere[] = [$item['filter'], $item['op'], $item['value']];
            }
        }
        $data = $this->model
