/*
 * 整合easyadmin项目的easy-admin.js文件
 * @link https://github.com/zhongshaofa/easyadmin
 * @link https://gitee.com/zhongshaofa/easyadmin
 * @link https://www.layui.com/doc/base/infrastructure.html
 *
 * easyadmin修改了layui.all.js和table.js文件代码：
 * return "function" == typeof e.templet ? e.templet(l) : i(t(e.templet).html() || String(a)).render(l) 的 e.templet(l) 为 e.templet(l, e)
 */
layui.extend({
    tableSelect: 'lay-module/tableSelect/tableSelect.min',
    xmSelect: 'lay-module/xmSelect/xm-select',
}).define(["form", "layer", "table", "laydate", "upload", "element", "laytpl", "jquery", "tableSelect", "xmSelect"], function (exports) {
    //调试代码 待删除！
    var CONFIG = {
        IS_SUPER_ADMIN: true,
    };
    ...
    var init = {
        ...
        upload_url: "dishes.upload/local",
        upload_exts: 'doc|gif|ico|icon|jpg|mp3|mp4|p12|pem|png|rar',
        upload_attachment_url: "dishes.upload/index",
        upload_success_code: 1,
        upload_icon_dir: '/static/images/upload-icons',
        upload_editor_url: 'dishes.upload/editor'
    };
    ...
    var admin = {
        ...
        table: {
            ...
            /**
             * 初始化表格搜索
             * @link http://easyadmin.99php.cn/docs/frontend/table.html#%E6%89%A9%E5%B1%95%E5%88%97%E5%8F%82%E6%95%B0
             * @param cols 列参数
             * @param elem table容器或者dom
             * @param tableId 容器唯一 id
             */
                    ...
                    $.each(uploadList, function (i, v) {
                        ...
                        upload.render({
                            ...
                            multiple: uploadNumber !== 'one',//是否多文件上传
                                ...
                                if (res.code === init.upload_success_code) {
                        ...
                        $(elem).bind("input propertychange", function (event) {
                            ...
                            if (urlString.length > 0) {
                                ...
                                $.each(urlArray, function (i, v) {
                                    liHtml += '<li><a><img src="' + v + '" data-image  onerror="this.src=\'' + init.upload_icon_dir + '/' + uploadIcon + '.png\';this.onerror=null"></a><small class="uploads-delete-tip bg-red badge" data-upload-delete="' + uploadName + '" data-upload-url="' + v + '" data-upload-sign="' + uploadSign + '">×</small></li>\n';
                                });
                ...
                if (uploadSelectList.length > 0) {
                    $.each(uploadSelectList, function (i, v) {
                        ...
                        tableSelect.render({
                            ...
                            searchType: 'more',
                            searchList: [
                                {searchKey: 'name', searchPlaceholder: '请输入文件名'},
                            ],
                            table: {
                                url: admin.url(init.upload_attachment_url),
                                cols: [[
                                    {type: selectCheck},
                                    {field: 'id', title: 'ID'},
                                    {field: 'url', minWidth: 100, search: false, title: '图片信息', imageHeight: 40, align: "center", templet: admin.table.image},
                                    {field: 'name', width: 150, title: '文件原名', align: "center"},
                                    {field: 'mime', width: 120, title: 'mime类型', align: "center"},
                ...
                if (editorList.length > 0) {
                    $.each(editorList, function (i, v) {
                        CKEDITOR.replace(
                            ...
                            {
                                height: $(this).height(),
                                filebrowserImageUploadUrl: admin.url(init.upload_editor_url),
                            });
                    });
                }
            },
